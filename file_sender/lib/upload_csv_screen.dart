        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              ...
              onPressed: () => startServerAndBroadcastIp(context),
...
void startServerAndBroadcastIp(BuildContext context) async {
  try {
    setState(() => _isUploading = true);
    ...
    // Find a suitable network interface (usually the WiFi or Ethernet)
    final interface = interfaces.firstWhere(
      (element) => !element.name.contains('lo'),
    ...
    // Start the server
    // final serverPath = 'Server_regNoSend.py'; // Update with actual path
    Process.start('python', [serverPath]).then((Process process) {
      ...
      Config.updateServerUrl('http://$ip:5000');
      
      // Broadcast the server IP
      broadcastServerIp(ip);
      
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Server started at $ip')),
      );
    });
  } catch (e) {
    print('Error starting server or broadcasting IP: $e');
    ScaffoldMessenger.of(context).showSnackBar(
...
void broadcastServerIp(String ip) async {
  try {
    RawDatagramSocket socket = await RawDatagramSocket.bind(InternetAddress.anyIPv4, 8888);
    socket.broadcastEnabled = true;
    final result = socket.send(ip.codeUnits, InternetAddress('255.255.255.255'), 8888);
    print('Broadcast result: $result bytes sent');
    
    // Keep broadcasting every 5 seconds
    Timer.periodic(Duration(seconds: 5), (timer) {
      socket.send(ip.codeUnits, InternetAddress('255.255.255.255'), 8888);
    });
  } catch (e) {
    print('Error broadcasting IP: $e');
  }
}