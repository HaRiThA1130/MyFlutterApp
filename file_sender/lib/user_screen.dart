...
class _UserScreenState extends State<UserScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Start listening for server IP broadcasts
    listenForServerIp();
  }

  void listenForServerIp() async {
    try {
      ...
      socket.listen((event) {
        if (event == RawSocketEvent.read) {
          ...
          if (datagram != null) {
            String serverIp = String.fromCharCodes(datagram.data);
            print("Detected Server IP: $serverIp");
            Config.updateServerUrl("http://$serverIp:5000");
            
            // Show a snackbar to inform the user
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Server detected at $serverIp')),
              );
            }
          }
        }
      ...
      print('Error listening for server IP: $e');
    }
  }
